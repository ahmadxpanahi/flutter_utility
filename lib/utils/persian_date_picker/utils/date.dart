...
class PersianDateUtils {
    ...
    switch (disableTypeData) {
      ...
      case 'List<String>':
        for (var i = 0; i < disable.length; i++) {
          inDisable = isValidDate(date) ? _isDisableDate(date, disable[i]) : false;
          if (inDisable) break;
        }
        break;
  ...
  bool _inDisableMonthList(month) {
    ...
    switch (disableTypeData) {
      case 'String':
        inDisable = monthNums.indexOf(int.parse(month)) != -1 ? (int.parse(month) == int.parse(disable)) : false;
        break;
      case 'List<String>':
        for (var i = 0; i < disable.length; i++) {
          inDisable = monthNums.indexOf(int.parse(month)) != -1 ? (int.parse(month) == int.parse(disable[i])) : false;
          if (inDisable) break;
        }
        break;
      default:
  ...
  bool _inDisableYearList(year) {
    ...
    switch (disableTypeData) {
      case 'String':
        inDisable = (0 < int.parse(year) && int.parse(year) < 2000) ? (int.parse(year) == int.parse(disable)) : false;
        break;
      case 'List<String>':
        for (var i = 0; i < disable.length; i++) {
          inDisable = (0 < int.parse(year) && int.parse(year) < 2000) ? (int.parse(year) == int.parse(disable[i])) : false;
          if (inDisable) break;
        }
        break;
      default:
  ...
  bool _inDisableTimeList(time) {
    ...
    switch (disableTypeData) {
      case 'String':
        inDisable = isValidTime(time) && isValidTime(disable) ? (comparTime(time, disable) == 0) : false;
        break;
      case 'List<String>':
        for (var i = 0; i < disable.length; i++) {
          inDisable = isValidTime(time) && isValidTime(disable[i]) ? (comparTime(time, disable[i]) == 0) : false;
          if (inDisable) break;
        }
        break;
      case '_GrowableList<String>':
