...
class OTPTextField extends StatefulWidget {
  ...
  OTPTextField({Key? key, this.length = 4, this.width = 10, this.controller, this.fieldWidth = 30, this.margin: const EdgeInsets.symmetric(horizontal: 3), this.otpFieldStyle, this.keyboardType = TextInputType.number, this.style = const TextStyle(), this.outlineBorderRadius: 10, this.textFieldAlignment = MainAxisAlignment.spaceBetween, this.obscureText = false, this.fieldStyle = FieldStyle.underline, this.onChanged, this.onCompleted}) : assert(length > 1);
...
class _OTPTextFieldState extends State<OTPTextField> {
  ...
  Widget buildTextField(BuildContext context, int i) {
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(counterText: "", border: _getBorder(_otpFieldStyle.borderColor), focusedBorder: _getBorder(_otpFieldStyle.focusBorderColor), enabledBorder: _getBorder(_otpFieldStyle.enabledBorderColor), disabledBorder: _getBorder(_otpFieldStyle.disabledBorderColor), errorBorder: _getBorder(_otpFieldStyle.errorBorderColor)),
  ...
  InputBorder _getBorder(Color color) {
    return widget.fieldStyle == FieldStyle.box ? OutlineInputBorder(borderSide: BorderSide(color: color), borderRadius: BorderRadius.circular(widget.outlineBorderRadius)) : UnderlineInputBorder(borderSide: BorderSide(color: color));
  }
...
class OtpFieldController {
  ...
  void set(List<String> pin) {
    ...
    if (pin.length < textFieldLength) {
      throw new Exception("Pin length must be same as field length. Expected: $textFieldLength, Found ${pin.length}");
    }
  ...
  void setValue(String value, int position) {
    ...
    if (position > maxIndex) {
      throw new Exception("Provided position is out of bounds for the OtpTextField");
    }
...
class OtpFieldStyle {
  ...
  OtpFieldStyle({this.backgroundColor: Colors.transparent, this.borderColor: Colors.black26, this.focusBorderColor: Colors.blue, this.disabledBorderColor: Colors.grey, this.enabledBorderColor: Colors.black26, this.errorBorderColor: Colors.red});
}
